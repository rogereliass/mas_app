...
Current lessons:
- 2026-03-29 | Home notifications could appear empty on real devices while emulator showed data | Notification fetch trusted cached `profile.id`; persisted identity snapshots on device can be stale/empty during bootstrap, causing recipient query misses | Revalidate canonical profile id from `auth user_id` before loading notifications and force refresh when opening the notifications panel if the list is empty
- 2026-03-25 | Offline app reopen could loop on profile loading and block meetings/library despite offline banner | Auth bootstrap and route guards depended on live profile fetch and had no persisted identity/roles hydration path | Hydrate cached identity snapshot (profile+roles) before network calls, make guards offline-aware, and allow shell/tabs when cached profile exists
