...
class AdminService with ScopedServiceMixin {
  ...
  Future<List<PendingProfile>> fetchPendingProfiles({
    ...
    try {
      debugPrint(
        '🔍 Fetching pending profiles for user (rank ${currentUser.roleRank})',
      );
      ...
      final profiles = <PendingProfile>[];
      for (final row in (response as List)) {
        try {
          profiles.add(PendingProfile.fromJson(row as Map<String, dynamic>));
        } catch (error) {
          final profileId = row is Map<String, dynamic> ? row['id'] : null;
          debugPrint(
            '⚠️ Skipping invalid pending profile row. id=$profileId error=$error',
          );
        }
      }
  ...
  Future<List<ProfileApproval>> fetchProfileApprovals(String profileId) async {
    try {
      ...
      final approvals = <ProfileApproval>[];
      for (final row in (response as List)) {
        try {
          approvals.add(ProfileApproval.fromJson(row as Map<String, dynamic>));
        } catch (error) {
          final approvalId = row is Map<String, dynamic> ? row['id'] : null;
          debugPrint(
            '⚠️ Skipping invalid profile approval row. id=$approvalId error=$error',
          );
        }
      }
  ...
  Future<void> _validateProfileAccess(
    ...
    if (currentUser.managedTroopId == null) {
      throw Exception(
        'SECURITY: Troop-scoped user has no managed troop assigned',
      );
    }
    ...
    if (profile.signupTroopId != currentUser.managedTroopId) {
      debugPrint(
        '🚨 SECURITY VIOLATION: User (rank ${currentUser.roleRank}, troop ${currentUser.managedTroopId}) '
  ...
  Future<void> acceptProfile({
    ...
    try {
      ...
      debugPrint(
        '📋 Role records prepared: ${roleRecords.map((r) => '${r['role_id']}: troop=${r['troop_context']}').join(', ')}',
      );
  ...
  Future<void> addProfileComment({
    ...
    try {
      debugPrint(
        '📝 Adding/updating comment: profileId=$profileId, approvedBy=$approvedBy',
      );
